        ...
        <div className="absolute top-4 right-4 flex flex-col items-end pointer-events-none">
            ...
            <div className="flex flex-col gap-2">
                {photos.slice(-5).reverse().map(photo => (
                    <div key={photo.id} className="w-16 h-12 bg-gray-900 border border-gray-600 rounded relative overflow-hidden">
                        {photo.status === 'developed' ? (
                            <div className={`w-full h-full flex items-center justify-center text-xs text-center ${
                                photo.type === 'enemy' ? 'bg-red-900/50 text-red-200' : 'bg-blue-900/50 text-blue-200'
                            }`}>
                                {photo.type}
                            </div>
                        ) : (
