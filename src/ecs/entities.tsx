...
const PlayerRender = () => {
    ...
    useFrame((state, delta) => {
        ...
        if (keys.current['ArrowLeft'] || keys.current['KeyA']) targetX -= speed * dt;
        if (keys.current['ArrowRight'] || keys.current['KeyD']) targetX += speed * dt;
        ...
        // Jump
        if (form === 'kitten' && (keys.current['Space'] || keys.current['ArrowUp'])) {
        ...
        if (mesh.current) {
            ...
            state.camera.lookAt(entity.position.x, entity.position.y, entity.position.z - 5);

            // Rotation
            mesh.current.rotation.y = lerp(mesh.current.rotation.y, (targetX - entity.position.x) * -0.1, dt * 5);
        }
    });
        ...
        <ECS.Entities in={ECS.world.with('tag', 'enemyType')}>
            {(entity) => (
                <group position={entity.position}>
                    {entity.enemyType === 'snowman' && <Snowman position={[0,0,0]} onRegister={() => () => {}} />}
                    {entity.enemyType === 'polar_bear' && <PolarBear position={[0,0,0]} onRegister={() => () => {}} />}
                    {entity.enemyType === 'glitch_imp' && <GlitchImp position={[0,0,0]} onRegister={() => () => {}} />}
                </group>
            )}
        </ECS.Entities>
    );
...
export const CollectibleRenderer = () => {
    return (
        <ECS.Entities in={ECS.world.with('tag', 'collectibleType')}>
            {(entity) => (
                <group position={entity.position}>
                    {entity.collectibleType === 'cocoa' && <HotCocoa position={[0,0,0]} onRegister={() => () => {}} />}
                </group>
            )}
        </ECS.Entities>
    );
